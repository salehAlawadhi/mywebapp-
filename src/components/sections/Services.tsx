...
function ServiceCard({ service, config }: { service: { title: string, description: string }, config: typeof serviceConfig[0] }) {
...
export default function ServicesSection() {
  ...
  return (
    <section id="services" className="relative min-h-screen py-40 flex flex-col justify-center items-center overflow-hidden px-4 md:px-8 bg-transparent border-t border-white/[0.02]">
      ...
      <div className="w-full max-w-[85rem] z-10 flex flex-col gap-32">
        ...
        <PhysicalReveal className="text-center space-y-10 max-w-4xl mx-auto" direction="up" amount={50}>
          ...
          <h3 className="text-5xl md:text-6xl lg:text-[5rem] font-bold tracking-tight text-zinc-100 font-[family-name:var(--font-space-grotesk)] leading-[1.05]">
            {t.services[lang].header_title} <br />
            <span className="italic text-zinc-500 font-light">{t.services[lang].header_italic}</span>
          </h3>
          <p className="text-lg md:text-xl text-zinc-500 leading-relaxed font-[family-name:var(--font-inter)] font-light max-w-2xl mx-auto tracking-wide">
            {t.services[lang].header_desc}
          </p>
        </PhysicalReveal>

        {/* Engine Modules (Bento Box Grid) */}
        <div className="grid grid-cols-1 md:grid-cols-4 auto-rows-[minmax(300px,auto)] gap-8 lg:gap-10">
          {t.services[lang].items.map((service, index) => (
            <ServiceCard key={index} service={service} config={serviceConfig[index]} />
          ))}
        </div>
      </div>
    </section>
